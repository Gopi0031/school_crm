...
export async function POST(request) {
  try {
    ...
    for (let i = 0; i < rawRows.length; i++) {
      ...
      const student = await prisma.student.findFirst({ where: { rollNo: row.rollno, branch } });
      if (!student) {
      ...
      await prisma.report.upsert({
        where:  { studentId_subject_exam: { studentId: student.id, subject: row.subject, exam } },
        update: { marksObtained, totalMarks, percentage, status: percentage >= 35 ? 'Pass' : 'Fail', academicYear },
        create: {
          studentId: student.id, studentName: student.name,
          rollNo: student.rollNo, class: student.class,
          section: student.section, branch,
          subject: row.subject, marksObtained, totalMarks,
          percentage, status: percentage >= 35 ? 'Pass' : 'Fail',
          exam, academicYear,
        },
      });
      inserted++;
    ...
    return NextResponse.json({ success: inserted > 0, message: `${inserted} report(s) saved, ${skipped} skipped.`, inserted, skipped, errors });
  } catch (err) {
