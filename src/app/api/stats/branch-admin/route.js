...
export async function GET(request) {
  try {
    ...
    // ── Students ──
    const students = await db.collection('students').find(branchFilter).toArray();
    ...
    // ── Teachers ──
    const teachers = await db.collection('teachers').find(branchFilter).toArray();
    const totalTeachers = teachers.length;

    // ── Reports ──
    const reports = await db.collection('reports').find(branchFilter).toArray();
    const passCount = reports.filter(r => /^pass$/i.test(r.result || r.status || '')).length;
    const failCount = reports.filter(r => /^fail$/i.test(r.result || r.status || '')).length;
