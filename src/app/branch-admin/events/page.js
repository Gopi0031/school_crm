...
import {
  ...
  CalendarDays, X, ImagePlus, Sparkles, Globe
} from 'lucide-react';
...
const BLANK = {
  name:'', class:'All', section:'All', date:'', startTime:'', endTime:'',
  description:'', academicYear:'2025-26', eventType:'General',
  visibility:['All'], posterImage:null, posterPublicId:null, isPublished:true,
};
...
export default function BranchAdminEvents() {
  ...
  // ✅ useAuth gives user directly — no localStorage needed
  useEffect(() => {
  ...
  const loadEvents = async () => {
    setLoading(true);
    try {
      const res  = await fetch(`/api/events?branch=${encodeURIComponent(user.branch)}&published=true`);
  ...
  // branch-admin: can only modify events scoped to their branch (not school-wide)
  const canModify = ev => !isSchoolWide(ev) && ev.branch === user?.branch;
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setForm(p => ({ ...p, posterImage:data.url, posterPublicId:data.publicId }));
  ...
  const resetForm = () => { setForm(BLANK); setImagePreview(null); };
  ...
  const save = async () => {
    ...
    try {
      ...
      if (d.success) {
        await loadEvents(); setShowAdd(false); setEditEvent(null); resetForm();
  ...
  const del = async () => {
    ...
    if (ev?.posterPublicId) {
      await fetch('/api/upload', { method:'DELETE', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ publicId:ev.posterPublicId }) });
    }
    await fetch(`/api/events/${deleteId}`, { method:'DELETE' });
    setDeleteId(null); await loadEvents(); showToast('🗑️ Event deleted');
  };

  const openEdit = ev => {
    setEditEvent(ev);
    setForm({
      name:ev.name, class:ev.class, section:ev.section, date:ev.date,
      startTime:ev.startTime||'', endTime:ev.endTime||'',
      description:ev.description||'', academicYear:ev.academicYear||'2025-26',
      eventType:ev.eventType||'General', visibility:ev.visibility||['All'],
      posterImage:ev.posterImage||null, posterPublicId:ev.posterPublicId||null,
  ...
  return (
    <AppLayout requiredRole="branch-admin">
      ...
      <PageHeader title="Branch Events" subtitle={`Managing events for ${user?.branch || 'your branch'}`}>
        <button className="btn btn-primary" onClick={() => { setEditEvent(null); resetForm(); setShowAdd(true); }}>
                    ...
                    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
                      ...
                      {canModify(ev) && (
                        <div style={{ display:'flex', gap:4 }}>
                          <button className="btn btn-outline" style={{ padding:'3px 6px' }} onClick={() => openEdit(ev)}><Edit2 size={10}/></button>
                          <button className="btn btn-danger"  style={{ padding:'3px 6px' }} onClick={() => setDeleteId(ev.id)}><Trash2 size={10}/></button>
                        </div>
                      )}
                    </div>
                  </div>
                    ...
                    <div style={{ display:'flex', justifyContent:'space-between', marginTop:6 }}>
                      ...
                      {canModify(ev) && (
                        <div style={{ display:'flex', gap:3 }}>
                          <button onClick={() => openEdit(ev)} style={{ background:`${c}15`, border:'none', cursor:'pointer', color:c, width:22, height:22, borderRadius:6, display:'flex', alignItems:'center', justifyContent:'center' }}><Edit2 size={9}/></button>
                          <button onClick={() => setDeleteId(ev.id)} style={{ background:'#fee2e220', border:'none', cursor:'pointer', color:'#f43f5e', width:22, height:22, borderRadius:6, display:'flex', alignItems:'center', justifyContent:'center' }}><Trash2 size={9}/></button>
                        </div>
                      )}
                    </div>
                  </div>
      ...
      {/* Add/Edit Modal */}
      <Modal open={showAdd} onClose={() => { setShowAdd(false); setEditEvent(null); resetForm(); }} title={editEvent ? 'Edit Event' : 'Add New Event'} size="md">
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
          ...
          <FormField label="Section">
            <select className="select" value={form.section} onChange={e => setForm({...form,section:e.target.value})}>
              {['All','A','B','C','D','E'].map(s => <option key={s}>{s}</option>)}
            </select>
          </FormField>
          ...
          <div style={{ gridColumn:'1/-1' }}>
            <FormField label="Event Poster">
              <div style={{ border:'2px dashed #e0e7ff', borderRadius:10, padding:14, background:'#fafbff', textAlign:'center' }}>
                {imagePreview || form.posterImage ? (
                  <div style={{ position:'relative', display:'inline-block' }}>
                    <img src={imagePreview||form.posterImage} alt="Poster" style={{ maxWidth:'100%', maxHeight:180, borderRadius:8 }}/>
                    <button onClick={() => { setForm(p=>({...p,posterImage:null,posterPublicId:null})); setImagePreview(null); }} style={{ position:'absolute', top:4, right:4, width:24, height:24, borderRadius:'50%', background:'#ef4444', color:'white', border:'none', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center' }}><X size={12}/></button>
                  </div>
                ) : (
                  <>
                    <input ref={fileInputRef} type="file" accept="image/*" onChange={handleImageUpload} style={{ display:'none' }} id="poster"/>
                    <label htmlFor="poster" style={{ cursor:'pointer', display:'flex', flexDirection:'column', alignItems:'center', gap:6 }}>
                      <div style={{ width:48, height:48, borderRadius:12, background:'linear-gradient(135deg,#6366f1,#8b5cf6)', display:'flex', alignItems:'center', justifyContent:'center' }}>
                        {uploading ? <span style={{ width:18, height:18, border:'2px solid rgba(255,255,255,0.3)', borderTopColor:'white', borderRadius:'50%', animation:'spin 0.7s linear infinite' }}/> : <ImagePlus size={20} color="white"/>}
                      </div>
                      <span style={{ fontSize:'0.8rem', color:'#6366f1', fontWeight:600 }}>{uploading?'Uploading...':'Click to upload'}</span>
                    </label>
                  </>
                )}
              </div>
          ...
          <div style={{ gridColumn:'1/-1' }}>
            <FormField label="Description">
              <textarea className="input" rows={2} value={form.description} onChange={e => setForm({...form,description:e.target.value})} placeholder="Details..." style={{ resize:'vertical' }}/>
            </FormField>
          </div>
        </div>
        <div style={{ display:'flex', gap:10, justifyContent:'flex-end', marginTop:16, paddingTop:12, borderTop:'1px solid #f0f2ff' }}>
          <button className="btn btn-outline" onClick={() => { setShowAdd(false); setEditEvent(null); resetForm(); }}>Cancel</button>
          <button className="btn btn-primary" onClick={save} disabled={saving||uploading}>{saving?'Saving...':editEvent?'Update':'Create'}</button>
        </div>
      </Modal>
      ...
      <Modal open={!!deleteId} onClose={() => setDeleteId(null)} title="Delete Event?" size="sm">
        <p style={{ color:'#64748b', textAlign:'center', margin:'16px 0' }}>This will permanently delete the event and its poster.</p>
