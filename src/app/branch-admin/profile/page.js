...
import { Camera, Save, Lock, CheckCircle, Eye, EyeOff, User, Shield } from 'lucide-react';
...
export default function BranchAdminProfile() {
  ...
  const handlePhoto = async (e) => {
    ...
    reader.onloadend = async () => {
      try {
        ...
        if (data.url) {
          setPreview(data.url);
          await fetch(`/api/users/${uid}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ profileImage: data.url, profileImagePublicId: data.publicId }) }); // ✅ uid
  ...
  const saveProfile = async () => {
    ...
    const res  = await fetch(`/api/users/${uid}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: form.email, phone: form.phone }) }); // ✅ uid
    const data = await res.json();
    if (data.success) {
  ...
  const savePwd = async () => {
    ...
    setSavingPwd(true);
    const res  = await fetch(`/api/users/${uid}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ password: pwd.newPwd }) }); // ✅ uid
    const data = await res.json();
    if (data.success) {
      ...
      <div style={{ maxWidth: 820, display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
          ...
          <div style={{ padding: '0 24px 20px' }}>
            <div style={{ position: 'relative', display: 'inline-block', marginTop: -36 }}>
              ...
              <label style={{ position: 'absolute', bottom: 0, right: 0, width: 26, height: 26, background: '#4f46e5', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', border: '2px solid white' }}>
            ...
            <div style={{ marginTop: 10 }}>
              ...
              <div style={{ fontSize: '0.75rem', color: '#64748b', marginTop: 3, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                <span style={{ background: '#eff6ff', color: '#4f46e5', padding: '2px 9px', borderRadius: 20, fontWeight: 600, fontSize: '0.7rem' }}>Branch Admin</span>
        ...
        <div className="card">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
            {[['Full Name', user?.name || ''], ['Branch', user?.branch || '']].map(([label, value]) => (
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', alignItems: 'center', marginTop: 16, paddingTop: 14, borderTop: '1px solid #f1f5f9' }}>
            {saved && <span style={{ color: '#10b981', display: 'flex', alignItems: 'center', gap: 5, fontSize: '0.83rem', fontWeight: 600 }}><CheckCircle size={14} /> Saved!</span>}
        ...
        <div className="card">
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 18 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: '0.7rem', color: '#94a3b8' }}>Minimum 8 characters</div>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
            <FormField label="New Password">
              <div style={{ position: 'relative' }}>
                ...
                <button onClick={() => setShowNew(p => !p)} style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: '#94a3b8', display: 'flex' }}>
            ...
            <FormField label="Confirm Password">
              <div style={{ position: 'relative' }}>
                ...
                <button onClick={() => setShowConf(p => !p)} style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: '#94a3b8', display: 'flex' }}>
          ...
          {pwd.newPwd.length > 0 && (
            <div style={{ marginTop: 10 }}>
              <div style={{ display: 'flex', gap: 4 }}>
                {[1, 2, 3, 4].map(s => (
                  <div key={s} style={{ flex: 1, height: 4, borderRadius: 3, background: s <= strength ? SC[strength] : '#e2e8f0', transition: 'background 0.3s' }} />
                ))}
              </div>
              <div style={{ fontSize: '0.72rem', color: SC[strength], fontWeight: 600, marginTop: 4 }}>{SL[strength]}</div>
            </div>
          )}
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', alignItems: 'center', marginTop: 16, paddingTop: 14, borderTop: '1px solid #f1f5f9' }}>
            {pwdSaved && <span style={{ color: '#10b981', display: 'flex', alignItems: 'center', gap: 5, fontSize: '0.83rem', fontWeight: 600 }}><CheckCircle size={14} /> Password updated!</span>}
            <button className="btn btn-primary" onClick={savePwd} disabled={savingPwd}>
              {savingPwd ? <><span style={{ width: 13, height: 13, border: '2px solid rgba(255,255,255,0.3)', borderTopColor: 'white', borderRadius: '50%', animation: 'spin 0.7s linear infinite', display: 'inline-block' }} /> Updating...</> : <><Lock size={13} /> Update Password</>}
            </button>
          </div>
        </div>
