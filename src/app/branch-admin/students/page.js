...
import { Eye, Plus, Trash2, Edit2, Search, Printer, FileDown, Upload, ClipboardList, X, IndianRupee } from 'lucide-react';

const BLANK = {
  name: '', rollNo: '', class: '', section: '',
  gender: 'Male', parentName: '', phone: '', email: '',
...
export default function BranchAdminStudents() {
  ...
  // ── Load fee structures for the branch ──
  const loadFeeStructures = async () => {
    if (!user?.branch) return;
    setFeeLoading(true);
    try {
      const academicYear = form.academicYear || '2025-26';
      const res = await fetch(`/api/fee-structure?branch=${encodeURIComponent(user.branch)}&academicYear=${academicYear}`);
      const data = await res.json();
      if (data.success) {
        setFeeStructures(data.data || []);
        console.log('📋 Loaded fee structures:', data.data?.length || 0);
      }
    } catch (err) {
  ...
  // Load fee structures when modal opens or academic year changes
  useEffect(() => {
    if (showAdd && user?.branch) {
      loadFeeStructures();
    }
  }, [showAdd, user?.branch, form.academicYear]);

  // ── Auto-fill fee when class changes ──
  const handleClassChange = (newClass) => {
    setForm(prev => {
      const updated = { ...prev, class: newClass };
      
      // Only auto-fill if NOT editing and fee structures are loaded
      if (!editStudent && feeStructures.length > 0) {
        const feeStructure = feeStructures.find(f => f.class === newClass);
        if (feeStructure) {
          updated.totalFee = feeStructure.totalFee || '';
          console.log(`✅ Auto-filled fee for ${newClass}: ₹${feeStructure.totalFee}`);
        } else {
          // Clear fee if no structure found
          updated.totalFee = '';
          console.log(`⚠️ No fee structure found for ${newClass}`);
        }
      }
  ...
  // Get fee structure info for current class
  const getCurrentFeeStructure = () => {
    if (!form.class || !feeStructures.length) return null;
    return feeStructures.find(f => f.class === form.class);
  };
  ...
  const saveStudent = async () => {
    ...
    setSaving(true);
    try {
      // ✅ Ensure totalFee is a proper integer (no floating point issues)
      const totalFee = Math.round(Number(form.totalFee) || 0);
      
      const payload = { 
        ...form, 
        totalFee,  // ✅ Use rounded integer
        branch: user?.branch, 
 ...
 const deleteStudent = async () => {
  try {
    console.log('🗑️ Deleting student:', deleteId);
  ...
  const handleBulkUpload = async () => {
    ...
    try {
      ...
      if (!text) { 
        setBulkResult({ success: false, message: 'Server returned empty response.' }); 
        return; 
      }
      let d;
      try { 
        ...
        setBulkResult({ success: false, message: `Invalid response: ${text.slice(0,100)}` }); 
        return; 
      }
      setBulkResult(d);
      if (d.success) { 
        load(); 
        showToast(`✓ ${d.inserted} student(s) uploaded`); 
      }
    } catch (err) {
  ...
  const currentFeeStructure = getCurrentFeeStructure();
    ...
    <AppLayout requiredRole="branch-admin">
      ...
      {toast.msg && (
        <div style={{ 
          ...
          {toast.msg}
          <button onClick={() => setToast({ msg: '' })} style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: 'white', padding: 0, marginLeft: 4 }}>
            <X size={14} />
          </button>
        </div>
      )}
      ...
      <Modal 
        open={showAdd} 
        onClose={() => { setShowAdd(false); setEditStudent(null); setError(''); }}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          ...
          <F label="Class" req>
            ...
            >
              ...
              {CLASSES.map(c => {
                const feeInfo = feeStructures.find(f => f.class === c);
                return (
                  <option key={c} value={c}>
                    {c} {feeInfo ? `(₹${feeInfo.totalFee.toLocaleString()})` : ''}
                  </option>
                );
              })}
            </select>
          ...
          {/* ✅ Total Fee with auto-fill indicator */}
          <F label="Total Fee (₹)">
            <div style={{ position: 'relative' }}>
              <input
                ...
                style={{ paddingRight: currentFeeStructure ? 80 : 12 }}
              />
              {currentFeeStructure && !editStudent && (
                <span style={{
            ...
            {form.class && !editStudent && (
              <div style={{
                ...
                background: currentFeeStructure ? '#f0fdf4' : '#fef9c3',
                border: `1px solid ${currentFeeStructure ? '#bbf7d0' : '#fde68a'}`,
                fontSize: '0.75rem',
                color: currentFeeStructure ? '#166534' : '#92400e',
              }}>
                {feeLoading ? (
                  '⏳ Loading fee structure...'
                ) : currentFeeStructure ? (
                  <>
                    <IndianRupee size={11} style={{ display: 'inline', verticalAlign: 'middle' }} />
                    <strong> {currentFeeStructure.totalFee.toLocaleString()}</strong> set for {form.class} ({form.academicYear})
                    {currentFeeStructure.term1Fee > 0 && (
                      <span style={{ color: '#64748b', marginLeft: 8 }}>
                        T1: ₹{currentFeeStructure.term1Fee.toLocaleString()} | 
                        T2: ₹{currentFeeStructure.term2Fee.toLocaleString()} | 
                        T3: ₹{currentFeeStructure.term3Fee.toLocaleString()}
                      </span>
                    )}
                  </>
                ) : (
                  <>⚠️ No fee structure found for {form.class}. <a href="/branch-admin/fee-structure" style={{ color: '#0891b2', textDecoration: 'underline' }}>Set it up</a></>
                )}
              </div>
            )}
          </F>
          ...
          <F label="Academic Year">
            <select className="select" value={form.academicYear}
      ...
      {/* Rest of your modals (Detail, Delete, Bulk Upload, Credentials) remain the same */}
      {/* ... */}
          ...
          <div style={{ display: 'flex', gap: 3, marginBottom: 18, background: '#f1f5f9', borderRadius: 10, padding: 3 }}>
            {TABS.map(t => (
              <button key={t} onClick={() => setSelectedTab(t.toLowerCase())}
                style={{ 
                  flex: 1, padding: '7px 4px', borderRadius: 8, border: 'none', cursor: 'pointer', fontSize: '0.78rem',
          ...
          <div>
            {selectedTab === 'details' && (
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2 }}>
                {[
                  ...
                  ['Year of Joining', selected.yearOfJoining],
            ...
            {selectedTab === 'attendance' && (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, marginBottom: 20 }}>
                {[
                  { l: 'Present', v: selected.presentDays || 0, c: '#10b981' },
                  { l: 'Absent', v: selected.absentDays || 0, c: '#f87171' },
