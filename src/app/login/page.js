...
export default function LoginPage() {
 ...
 const handleLogin = async (e) => {
  ...
  const hints = [
    { role: 'Super Admin',  u: 'superadmin',   p: 'Admin@1234', color: '#818cf8' },
    { role: 'Branch Admin', u: 'gopichand',     p: 'Gopi1234',  color: '#34d399' },
    { role: 'Teacher',      u: 'gc.koritepadu', p: 'gc123456',  color: '#f472b6' },
    { role: 'Student',      u: 'krtgnt001',     p: 'std@001',   color: '#fb923c' },
  ];
  ...
  {loading ? (
    ...
    <><LogIn size={17} /> Sign In to Dashboard</>
        ...
        <div style={{
          ...
          animation: mounted ? 'fadeSlideUp 0.9s ease' : 'none',
        }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
            {hints.map(({ role, u, p, color }) => (
              <div
                ...
                onMouseEnter={e => { e.currentTarget.style.background = `${color}15`; e.currentTarget.style.borderColor = `${color}60`; e.currentTarget.style.transform = 'translateY(-1px)'; }}
                onMouseLeave={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.03)'; e.currentTarget.style.borderColor = `${color}30`; e.currentTarget.style.transform = 'translateY(0)'; }}
              >
                <div style={{ fontSize: '0.72rem', fontWeight: 700, color, marginBottom: 3 }}>{role}</div>
