...
export default function SuperAdminBranches() {
  ...
  const resetPassword = async () => {
    ...
    setSaving(true);
    try {
      const r = await fetch(`/api/branches/${showReset._id}/reset-password`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        ...
        <TableWrapper>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={8}><EmptyState message="No branches found. Create one!" /></td></tr>
            ) : filtered.map((b, i) => (
              <tr key={b._id}>
                <td style={{ color: '#94a3b8' }}>{i + 1}</td>
                <td style={{ fontWeight: 700 }}>{b.name}</td>
                <td>{b.adminId?.name || '—'}</td>
                <td style={{ color: '#4f46e5', fontWeight: 600 }}>{b.adminId?.username || '—'}</td>
                <td>{b.adminId?.email || b.email || '—'}</td>
                <td>{b.adminId?.phone || b.phone || '—'}</td>
                <td>
                  <span style={{
