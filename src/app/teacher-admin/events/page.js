...
const BLANK  = { name:'', class:'All', section:'All', date:'', startTime:'', endTime:'', description:'', academicYear:'2024-25' };

const EVENT_COLORS = ['#4f46e5','#0891b2','#10b981','#f59e0b','#ef4444','#8b5cf6','#ec4899'];
...
export default function TeacherEvents() {
  ...
  const save = async () => {
    ...
    const d = await r.json();
    if (d.success) { load(); setShowAdd(false); setEditEvent(null); setForm(BLANK); }
    setSaving(false);
  };
  ...
  const totalThisMonth = filtered.filter(e => e.date?.startsWith(`${year}-${String(month + 1).padStart(2,'0')}`)).length;

  return (
    <AppLayout requiredRole="teacher-admin">
      <style jsx global>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style>

      <PageHeader title="Events" subtitle={`${user?.branch} events`}>
        <button className="btn btn-primary" style={{ display:'flex', alignItems:'center', gap:6 }}
          onClick={() => { setEditEvent(null); setForm({ ...BLANK, branch: user?.branch }); setShowAdd(true); }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:12, marginBottom:16 }}>
        {[
          { l:'Total Events',    v: filtered.length,      c:'#4f46e5' },
          { l:'This Month',      v: totalThisMonth,        c:'#0891b2' },
      ...
      <div className="card" style={{ marginBottom:14, padding:'12px 16px' }}>
        <div style={{ display:'flex', alignItems:'center', gap:8, background:'#f8fafc', border:'1.5px solid #e2e8f0', borderRadius:9, padding:'8px 12px', maxWidth:320 }}>
          ...
          />
          {search && (
            <button onClick={() => setSearch('')} style={{ background:'none', border:'none', cursor:'pointer', color:'#94a3b8', display:'flex', padding:0 }}><X size={13} /></button>
          )}
        </div>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 290px', gap:18 }}>
        ...
        <div className="card">
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(7,1fr)', gap:3, marginBottom:4 }}>
            {DAYS.map((d, i) => (
              <div key={d} style={{ textAlign:'center', padding:'5px 0', fontSize:'0.7rem', fontWeight:700, color: i === 0 || i === 6 ? '#ef4444' : '#94a3b8' }}>{d}</div>
            ))}
          </div>
            ...
            {Array.from({ length: daysInMonth }, (_, i) => {
              ...
              return (
                <div key={day} onClick={() => setSelectedDate(isSel ? null : day)}
                  style={{
                    minHeight: 58, border:`1.5px solid ${isSel ? '#4f46e5' : isToday ? '#bfdbfe' : '#e2e8f0'}`,
                    borderRadius:9, padding:'5px 6px', cursor:'pointer',
                    background: isSel ? '#eef2ff' : isToday ? '#f0f9ff' : 'white',
                    transition:'all 0.15s',
                  }}>
                  <div style={{ fontWeight: isToday ? 800 : 500, fontSize:'0.78rem', color: isToday ? '#4f46e5' : '#1e293b',
                    width:20, height:20, borderRadius:'50%', background: isToday ? '#4f46e5' : 'transparent',
                    color: isToday ? 'white' : '#1e293b', display:'flex', alignItems:'center', justifyContent:'center' }}>
                    {day}
                  </div>
          ...
          {selectedDate && (
            <div style={{ marginTop:16, paddingTop:16, borderTop:'1px solid #f1f5f9' }}>
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:10 }}>
                <h4 style={{ fontWeight:700, fontSize:'0.9rem', color:'#1e293b', display:'flex', alignItems:'center', gap:6 }}>
                  <Calendar size={15} color="#4f46e5" /> {selectedDate} {MONTHS[month]}
                </h4>
                <button onClick={() => setSelectedDate(null)} style={{ background:'none', border:'none', cursor:'pointer', color:'#94a3b8', display:'flex' }}><X size={14} /></button>
              </div>
              {eventsOnDay(selectedDate).length === 0 ? (
                <div style={{ textAlign:'center', padding:'20px 0', color:'#94a3b8', fontSize:'0.83rem' }}>
                  No events this day
                  <div style={{ marginTop:8 }}>
                    <button className="btn btn-outline" style={{ fontSize:'0.75rem', padding:'4px 10px' }}
                      onClick={() => { const d=`${year}-${String(month+1).padStart(2,'0')}-${String(selectedDate).padStart(2,'0')}`; setForm({...BLANK,date:d}); setShowAdd(true); }}>
                ...
                <div key={ev._id} style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', padding:'10px 12px', borderRadius:9, background:'#f8fafc', marginBottom:6, borderLeft:`3px solid ${getColor(ev.name)}` }}>
                  ...
                  <div style={{ display:'flex', gap:4, flexShrink:0 }}>
                    <button className="btn btn-outline" style={{ padding:'3px 7px' }}
                      onClick={() => { setEditEvent(ev); setForm({ name:ev.name, class:ev.class, section:ev.section, date:ev.date, startTime:ev.startTime, endTime:ev.endTime, description:ev.description, academicYear:ev.academicYear }); setShowAdd(true); }}>
                      <Edit2 size={11} />
                    </button>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          <div className="card" style={{ padding:'16px' }}>
            <h3 style={{ fontWeight:800, marginBottom:12, fontSize:'0.9rem', color:'#1e293b', display:'flex', alignItems:'center', gap:7 }}>
              <Calendar size={15} color="#4f46e5" /> Upcoming Events
            </h3>
            {upcoming.length === 0 ? (
              ...
              <div key={ev._id} style={{ padding:'9px 0', borderBottom: i < upcoming.length-1 ? '1px solid #f1f5f9' : 'none' }}>
                <div style={{ display:'flex', gap:9, alignItems:'flex-start' }}>
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    ...
                    <div style={{ fontSize:'0.68rem', color:'#64748b', marginTop:2 }}>{ev.date} {ev.startTime ? `• ${ev.startTime}` : ''}</div>
                    <div style={{ fontSize:'0.65rem', color:'#94a3b8' }}>{ev.class !== 'All' ? `${ev.class} — ${ev.section}` : 'All classes'}</div>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
          ...
          <div style={{ gridColumn:'1/-1' }}>
            <FormField label="Description">
              <textarea className="input" rows={3} value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} style={{ resize:'vertical' }} placeholder="Optional event details..." />
            </FormField>
          </div>
        </div>

        {!form.name && <div style={{ background:'#fffbeb', border:'1px solid #fde68a', borderRadius:8, padding:'8px 12px', fontSize:'0.78rem', color:'#92400e', marginTop:4 }}>⚠️ Event name and date are required</div>}
