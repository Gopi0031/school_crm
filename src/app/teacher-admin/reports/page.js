...
import { Plus, Upload, Eye, Trash2, Search, FileDown, X, Edit2, CheckSquare, Square, Loader } from 'lucide-react';
...
function buildReportPDF({ studentReports, title, user, cls, section, exam }) {
  const groupedByStudent = studentReports.reduce((acc, r) => {
  ...
  const studentBlocks = Object.values(groupedByStudent).map(st => {
    const totalObtained = st.subjects.reduce((s, r) => s + Number(r.marksObtained), 0);
    ...
    const overallPass   = overallPct >= 35;
    const rows = st.subjects.map((r, i) => {
      const pct = r.percentage;
      const pctColor = pct >= 75 ? '#16a34a' : pct >= 35 ? '#f59e0b' : '#ef4444';
      return `<tr style="background:${i%2===0?'#fff':'#f8fafc'}">
        ...
        <td style="padding:7px 10px;border-bottom:1px solid #e2e8f0;text-align:center;font-weight:700;color:${pctColor}">${pct}%</td>
      ...
      <div style="margin-bottom:24px;page-break-inside:avoid">
        <div style="display:flex;justify-content:space-between;align-items:center;background:#f8fafc;border-left:4px solid #4f46e5;padding:10px 14px;border-radius:0 8px 8px 0;margin-bottom:10px">
          ...
          <div style="text-align:right">
            <div style="font-size:18px;font-weight:800;color:${overallPct>=75?'#10b981':overallPct>=35?'#f59e0b':'#ef4444'}">${overallPct}%</div>
            <span style="padding:2px 10px;border-radius:20px;font-size:11px;font-weight:700;background:${overallPass?'#dcfce7':'#fee2e2'};color:${overallPass?'#16a34a':'#dc2626'}">${overallPass?'PASS':'FAIL'}</span>
          </div>
        </div>
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          <thead>
            <tr style="background:#4f46e5">
              <th style="padding:8px 10px;text-align:left;color:white;font-weight:700">Subject</th>
              <th style="padding:8px 10px;text-align:center;color:white;font-weight:700">Marks</th>
              <th style="padding:8px 10px;text-align:center;color:white;font-weight:700">Total</th>
              <th style="padding:8px 10px;text-align:center;color:white;font-weight:700">%</th>
              <th style="padding:8px 10px;text-align:center;color:white;font-weight:700">Result</th>
            </tr>
          </thead>
          <tbody>${rows}</tbody>
          <tfoot>
            <tr style="background:#f1f5f9">
              <td style="padding:8px 10px;font-weight:800;color:#1e293b">TOTAL</td>
              <td style="padding:8px 10px;text-align:center;font-weight:800;color:#4f46e5">${totalObtained}</td>
              <td style="padding:8px 10px;text-align:center;color:#64748b">${totalMax}</td>
              <td style="padding:8px 10px;text-align:center;font-weight:800;color:${overallPct>=75?'#10b981':overallPct>=35?'#f59e0b':'#ef4444'}">${overallPct}%</td>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .sv{font-size:20px;font-weight:800}.sl{font-size:11px;margin-top:3px}
    ...
    @media print{
      body{padding:14px}
      .top,.stat{-webkit-print-color-adjust:exact;print-color-adjust:exact}
  ...
  <div class="footer">
    ...
    <span>Total Students: ${Object.keys(groupedByStudent).length} &nbsp;•&nbsp; Printed on ${new Date().toLocaleString('en-IN')}</span>
  </div>
  </body></html>`;
}
...
export default function TeacherReports() {
  ...
  const handleBulkUpload = async () => {
    ...
    formData.append('exam', bulkExam);
    formData.append('academicYear', bulkYear);
    try {
      const r    = await fetch('/api/reports/bulk-upload', { method: 'POST', body: formData });
  ...
  const downloadTemplate = () => {
    const csv = ['rollno,subject,marks,totalmarks,exam,academicyear','2025001,Mathematics,85,100,Annual,2025-26','2025002,Science,72,100,Annual,2025-26','2025003,English,90,100,Annual,2025-26'].join('\n');
  ...
  return (
    <AppLayout requiredRole="teacher-admin">
      ...
      {/* ── Toast ─────────────────────────────────────────── */}
      {toast.msg && (
        <div style={{ position: 'fixed', top: 20, right: 20, zIndex: 9999, background: toast.type === 'error' ? '#ef4444' : '#10b981', color: 'white', padding: '12px 20px', borderRadius: 12, fontWeight: 600, fontSize: '0.875rem', boxShadow: '0 8px 28px rgba(0,0,0,0.18)', display: 'flex', alignItems: 'center', gap: 8, animation: 'slideIn 0.3s ease' }}>
          {toast.msg}
      ...
      <PageHeader title="Reports" subtitle="Student academic results">
        ...
        {checkedIds.length > 0 && (
          <>
            ...
            <button className="btn btn-danger" onClick={() => setShowMultiDel(true)} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <Trash2 size={13} /> Delete ({checkedIds.length})
            </button>
          </>
        )}

        <button className="btn btn-outline" onClick={() => { setBulkFile(null); setBulkResult(null); setShowBulk(true); }} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <Upload size={13} /> Bulk Upload
        </button>
        <button className="btn btn-primary" onClick={() => { setForm(BLANK); setEditReport(null); setError(''); setShowAdd(true); }} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      {checkedIds.length > 0 && (
        <div style={{ background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: 10, padding: '10px 16px', marginBottom: 12, display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: '0.83rem' }}>
          <span style={{ color: '#1d4ed8', fontWeight: 600 }}>✓ {checkedIds.length} report(s) selected — you can Export or Delete them</span>
          <button onClick={() => setCheckedIds([])} style={{ background: 'none', border: 'none', color: '#64748b', cursor: 'pointer', fontSize: '0.78rem' }}>Clear selection</button>
        </div>
      )}
      ...
      <Modal open={showAdd} onClose={() => { setShowAdd(false); setEditReport(null); setError(''); }}
        ...
        {error && (
          <div style={{ background: '#fee2e2', color: '#991b1b', padding: '9px 14px', borderRadius: 9, fontSize: '0.83rem', marginTop: 10 }}>⚠️ {error}</div>
        )}
      ...
      <Modal open={showBulk} onClose={() => { setShowBulk(false); setBulkFile(null); setBulkResult(null); }} title="Bulk Upload Reports" size="sm">
        <div style={{ textAlign: 'center', padding: '4px 0 12px' }}>
          ...
          <code style={{ background: '#f1f5f9', padding: '4px 10px', borderRadius: 6, fontSize: '0.73rem', color: '#1e293b' }}>
        ...
        <Modal open onClose={() => setSelected(null)} title={`Report — ${selected.studentName}`} size="sm">
          ...
          <div style={{ background: '#f8fafc', borderRadius: 10, padding: '14px 16px', marginBottom: 14, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              ...
              <div style={{ fontWeight: 800, fontSize: '1.4rem', color: '#1e293b' }}>{selected.marksObtained} <span style={{ fontSize: '0.9rem', color: '#64748b', fontWeight: 400 }}>/ {selected.totalMarks}</span></div>
            </div>
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontWeight: 800, fontSize: '2rem', color: selected.percentage >= 75 ? '#10b981' : selected.percentage >= 35 ? '#f59e0b' : '#ef4444' }}>{selected.percentage}%</div>
            </div>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2 }}>
            {[
              ['Subject', selected.subject], ['Exam', selected.exam],
              ['Year',    selected.academicYear], ['Class', `${selected.class}–${selected.section}`],
            ].map(([l, v]) => (
      ...
      <Modal open={!!deleteId} onClose={() => setDeleteId(null)} title="Delete Report?" size="sm">
        <div style={{ textAlign: 'center', padding: '8px 0 16px' }}>
          ...
          <p style={{ color: '#64748b', fontSize: '0.875rem' }}>This will permanently delete the report record. This cannot be undone.</p>
        </div>
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
